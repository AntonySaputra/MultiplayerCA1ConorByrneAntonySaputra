...
{
	return !mPlayerAircraft->isMarkedForRemoval() && !mPlayerAircraft2->isMarkedForRemoval();
}
...
void World::handleCollisions()
{
	...
	{
		if (matchesCategories(pair, CategoryID::PlayerAircraft1, CategoryID::EnemyAircraft))
		{
		...
		else if (matchesCategories(pair, CategoryID::PlayerAircraft1, CategoryID::Pickup))
		{
			auto& player = static_cast<Aircraft&>(*pair.first);
			...
			|| matchesCategories(pair, CategoryID::PlayerAircraft1, CategoryID::EnemyProjectile))
		{
...
void World::buildScene()
{
	...
	std::unique_ptr<Aircraft> player(new Aircraft(AircraftID::Eagle1, mTextures, mFonts));
	...
	mPlayerAircraft2->setPosition(mSpawnPosition + sf::Vector2f(100,0));
...
void World::adaptPlayerPosition()
{
	...
	sf::Vector2f position2 = mPlayerAircraft2->getPosition();
	...
	mPlayerAircraft2->setPosition(position2);
}
...
void World::adaptPlayerVelocity()
{
	...
	sf::Vector2f velocity2 = mPlayerAircraft2->getVelocity();

	// If moving diagonally, reduce velocity (to have always same velocity)
	if (velocity2.x != 0.f && velocity2.y != 0.f)
		mPlayerAircraft2->setVelocity(velocity2 / std::sqrt(2.f));

	// Add scrolling velocity
	mPlayerAircraft2->accelerate(0.f, mScrollSpeed);
}
