	...
	, mMissileDisplay(nullptr)
{
	...
	if (getCategory() == (static_cast<int>(CategoryID::PlayerAircraft1)))
	{
	...
	if (getCategory() == (static_cast<int>(CategoryID::PlayerAircraft2)))
	{
		std::unique_ptr<TextNode> missileDisplay(new TextNode(fonts, ""));
		missileDisplay->setPosition(0, 70);
		mMissileDisplay = missileDisplay.get();
...
unsigned int Aircraft::getCategory() const
{
	if (isAllied())
	{
		if (mType == AircraftID::Eagle1)
		{
			return static_cast<int>(CategoryID::PlayerAircraft1);
		}
			
		else
		{
			return static_cast<int>(CategoryID::PlayerAircraft2);
		}
...
{
	return mType == AircraftID::Eagle1 || mType == AircraftID::Eagle2;
}
